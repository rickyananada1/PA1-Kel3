                ...
                <form class="form-group py-5 col-11" method="post" action="/dashboard/action/sampah/update/{{$id}}">
                    ...
                    <div class="mb-5">
                        ...
                        <select type="text" name="tipe_sampah" class="form-select" required>
                            ...
                            @foreach($TipeSampah as $TipeSampahI)
                                <option
                                    {{ ($TipeSampahI->id == $RecentData->tipe_sampah) ? "selected='selected'" : "" }}
                                    value="{{ $TipeSampahI->id }}">
                                    {{ $TipeSampahI->nama_sampah }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-5">
                        ...
                        <div class="input-group">
                            <input type="number" min="0" name="jumlah" value="{{ $RecentData->amount }}" class="form-control" required>
                            <label for="jumlah" class="input-group-text">kg</label>
                        </div>
                    </div>
                    <div class="mb-5">
                        <input type="submit" value="ubah data" class="btn btn-success">
                        <a href="{{ route('sampah.home') }}" class="btn btn-secondary">kembali ke dashboard</a>
                    </div>
                </form>
            </div>
        </div>
